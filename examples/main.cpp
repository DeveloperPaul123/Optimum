...
float f(Optimum::Vector& vec, MatrixXd& cbct, MatrixXd& bli);
void testTransform();
void testGATransform();
void testNM1D();
void testGA1D();
void testGA2D();
float rosenbrock(Optimum::Vector v);
float gaRosenbrock(Chromosome<float> c);
float ga1DTest(Chromosome<float> c);
float nm1DTest(Optimum::Vector v);
float camelHumps(Optimum::Vector v);
...
int main(int argc, const char* argv[]) {
    ...
    //perform scaling.
    pSet.doScale = true;
    auto matcher = new Optimum::PointMatcher(pSet);
...
void testTransform() {
    ...
    NelderMeadMinimizer minimizer(dimension, precision);
    Optimum::Vector v(1.0f, 1.0f, 1.0f);
    minimizer.initialGuess(v);
    while (!minimizer.done()) {
        float score = f(v, A, B);
        cout << "Score: " << score << endl;
...
*/
float f(Optimum::Vector& vec, MatrixXd& cbct, MatrixXd& bli) {
    Optimum::Vector temp = Optimum::Vector(vec);
    float shift_x = vec[0];
    float shift_y = vec[1];
    float deg = vec[2];
