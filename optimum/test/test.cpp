...
TEST_F(ICPTester, testTransform) {
	...
	std::cout << "Best rotation: " << rot << std::endl;
	std::cout << "Best Transform: " << trans << std::endl;
	std::cout << "Error: " << error << std::endl;
	EXPECT_NEAR(4.0, error, 1.0);
}

TEST_F(ICPTester, testTransformIcp) {
	double arrA[21][3] = {
		{ -16.6728, 25.9217, 1.9945 },
		{ -1.60935, 26.4446, 1.9945 },
		{ 13.4151, 26.8788, 1.9945 },
	...
	Optimum::ICP icp(B, A, settings);
	icp.solve();
	Eigen::MatrixXd rot = icp.getBestRotation();
	Eigen::MatrixXd trans = icp.getBestTranslation();
	Eigen::MatrixXd transformed = Optimum::PointMatcher::applyTransformation(B, trans, rot);
